...
#    filename = './testImgs/FPK_01.jpg'
    ...
    opening = cv2.dilate(opening, kernel2, iterations=2)
    plot.imshow(opening)
...
def extract_img(location,img,contour=None):
    ...
    if (w > h):
        res = cv2.resize(extracted_img, (IMG_SIZE, (math.ceil)(h * IMG_SIZE / w)), interpolation=cv2.INTER_AREA)
        d = int(abs(res.shape[0] - res.shape[1]) / 2)
        black[d:res.shape[0] + d, 0:res.shape[1]] = res
    else:
        res = cv2.resize(extracted_img, ((math.ceil)(w * IMG_SIZE / h), IMG_SIZE), interpolation=cv2.INTER_AREA)
        d = int(abs(res.shape[0] - res.shape[1]) / 2)
