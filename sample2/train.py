...
def get_callbacks(output_subdir, training_model, prediction_model, val_generator):
    training_model_checkpoint = MultiGPUModelCheckpoint(os.path.join(output_subdir, cfg.training_model_cp_filename), training_model, save_best_only=cfg.save_best_only, save_weights_only=True, monitor='loss', mode='min')
    prediction_model_checkpoint = PredictionModelCheckpoint(os.path.join(output_subdir, cfg.prediction_model_cp_filename), prediction_model, save_best_only=cfg.save_best_only, save_weights_only=True, monitor='loss', mode='min')
    ...
    return [evaluator, tensorboard, training_model_checkpoint, prediction_model_checkpoint,  lr_reducer]
...
def load_weights_if_resume_training(training_model):
    if cfg.resume_training:
        ...
        training_model.load_weights(cfg.load_model_path, by_name = True)
...
#%%
    history = training_model.fit_generator(train_generator,
                                 ...
                                 callbacks=callbacks)
#    training_model.save_weights('./' + output_subdir + '/final.hdf5')
